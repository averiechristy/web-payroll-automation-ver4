            ...
            <div class="col-md-12 grid-margin">
                <div class="row">
                    <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                        <h3 class="font-weight-bold">Kontrak Karyawan</h3>
                    </div>
                </div>
                <div class="card shadow mb-4">
                    <div class="py-3">
                        <h5 class="description ml-3">Unggah file untuk menambahkan data kontrak baru, atau tambahkan ke kumpulan data kontrak yang sudah ada. <br> Anda dapat mengunggah file Excel</h5>
                        <div class="button-group ml-3">
                            <a href="{{ route('kontrak.create') }}">
                                <button type="button" class="btn btn-primary">Tambah Kontrak Karyawan</button>
                            </a>
                            <a href="{{ route('downloadkontrak') }}" id="download-template-link">
                                <button type="button" class="btn btn-warning" download>Unduh Template</button>
                            </a>
                        </div>
                        <div class="importdata ml-3">
                            <form id="upload-form" action="{{ route('importkontrak') }}" method="post" enctype="multipart/form-data" style="display: flex; align-items: center;">
                                @csrf
...
<tr>
    ...
    <th>Action</th>
                             ...
                             <tr>
                                ...
                                <td>{{ \Carbon\Carbon::parse($item->tanggal_awal_kontrak)->format('d - F - Y') }}</td>
                                <td>{{ \Carbon\Carbon::parse($item->tanggal_akhir_kontrak)->format('d - F - Y') }}</td>
                                <td>{{$item->created_by}}</td>
                                <td>{{$item->created_at}}</td>
                                <td>{{$item->updated_by}}</td>
                                <td>{{$item->updated_at}}</td>
                                <td>
                                            <a href="{{ route('showkontrak', $item->id) }}">
                                            ...
                                            <form id="deleteForm-{{ $item->id }}" method="POST" action="{{ route('deletekontrak', $item->id) }}">
                                                @csrf
