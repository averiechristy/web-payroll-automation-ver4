...
<div class="main-panel">
    <div class="content-wrapper">
        <div class="row">
            ...
            <div class="py-3 ml-3">
                        <h5 class ="font-weight-bold">Proses Laporan MAD</h5>
        ...
        <option value="{{ $currentYear }}">{{ $currentYear }}</option>
                ...
                <div id="myDataTable_filter" class="dataTables_filter">
                    ...
                    <thead>                                                    
                          ...
                          @foreach ($mad as $item )
                        <tr>
                           <td>{{$item->judul_mad}}</td>
                           <td>    
                                <a href="{{route('tampilmad', $item->id)}}">
                                        Lihat Detail               
                                </a>
                                </td>  

                           <td>{{$item->status_mad}}</td>
                           <td>{{$item -> created_by}}</td>
                           <td>{{$item -> created_at}}</td>
                           
<td>
    @if ($item->status_mad == "Closing")
    <form action="{{ route('batalkan.closing', $item->id) }}" method="POST">
        @csrf
    ...
    <script>
        ...
        document.querySelector('form[action="{{ route('exportmad') }}"]').addEventListener('submit', function(event) {
            ...
            if (month === "" || year === "") {
                ...
                // Check for alerts and display loading screen if none are found
