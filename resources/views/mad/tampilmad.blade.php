            ...
            <div class="col-md-12 grid-margin">
              <div class="row">
                <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                  <h3 class="font-weight-bold">{{$mad -> judul_mad}}</h3>
                </div>
              </div>
              ...
              <div class="card shadow mb-4">
                  ...
                  <div class="row mb-4">
      <div class="col-md-12">
          <a href="{{ route('download.mad', ['bulan' => $mad->bulan, 'tahun' => $mad->tahun, 'status_mad'=> $mad->status_mad,'dataMAD' => json_encode($dataLembur)]) }}" class="btn btn-success">Download MAD</a>
      </div>
  </div>

  <form id="closeMadForm" action="{{ route('close.mad', $mad->id)}}" method="POST" style="display: none;">
    @csrf
    <input type="hidden" name="dataLembur" id="dataLemburInput">
</form>

<script>
  function closeMad() {
    var dataLembur = @json($dataLembur); // Assuming $dataLembur is passed from your backend
    document.getElementById('dataLemburInput').value = JSON.stringify(dataLembur);
    document.getElementById('closeMadForm').submit();
}

</script>
<div class="row mb-4">
    <div class="col-md-12">
        @if ($mad->status_mad == 'Closing')
        <button type="button" class="btn btn-primary" disabled>Closing MAD</button>
        @else
        <button type="button" class="btn btn-primary" onclick="closeMad()">Closing MAD</button>
        @endif
    </div>
</div>
    ...
    <label for="search">search
        <input id="search" placeholder>
                    ...
                    <tr>
                        ...
                        <th>Subtotal</th>
                        <th>Biaya Manajemen (%)</th>
                        <th>Biaya Manajemen (besaran)</th>
            ...
            <tr>
            <td>{{ $lembur['no_amandemen'] }}</td>
            <td>{{ $lembur['nik'] }}</td>
                <td>{{ $lembur['nama_karyawan'] }}</td>
                <td>{{ $lembur['nama_penempatan'] }}</td>
                <td>{{ $lembur['nama_posisi'] }}</td>
                <td>{{ $lembur['kodepembayaran'] }}</td>
                <td>{{ $lembur['rcc'] }}</td>
                <td>{{ $lembur['tanggal_lembur'] }}</td>
                <td>{{ $lembur['jenis_hari'] }}</td>
                <td>@if ( $lembur['jenis_hari'] == "Kerja")
                K
                @elseif ( $lembur['jenis_hari'] == "Libur")
L
                @endif
</td>
<td>{{ $lembur['upah'] }}</td>
<td>{{ $lembur['tunjanganamount'] }}</td>
<td>{{ $lembur['jam_mulai'] }}</td>
<td>{{ $lembur['jam_selesai'] }}</td>
                <td>{{ $lembur['jumlah_jam_lembur'] }}</td>
           
                <td>{{ $lembur['jam_pertama'] }}</td>
                <td>{{ $lembur['jam_kedua'] }}</td>
                <td>{{ $lembur['jam_ketiga'] }}</td>
                <td>{{ $lembur['jam_keempat'] }}</td>
                <td>{{ 'Rp ' . number_format($lembur['biaya_jam_pertama'], 0, ',', '.') }}</td>

               
                <td>{{ 'Rp ' . number_format($lembur['biaya_jam_kedua'], 0, ',', '.') }}</td>
                <td>{{ 'Rp ' . number_format($lembur['biaya_jam_ketiga'], 0, ',', '.') }}</td>
                <td>{{ 'Rp ' . number_format($lembur['biaya_jam_keempat'], 0, ',', '.') }}</td>
                <td>{{ 'Rp ' . number_format($lembur['subtotal'], 0, ',', '.') }}</td>
                <td>{{ $lembur['management_fee'] *100}}</td>
                <td>{{ 'Rp ' . number_format($lembur['amount_management'], 0, ',', '.') }}</td>
                <td>{{ 'Rp ' . number_format($lembur['total_sebelum_ppn'], 0, ',', '.') }}</td>
                <td></td>
                <td></td>
                <td>{{ $lembur['kodeslid'] }}</td>
            </tr>
            @endforeach
                      </tbody>
                        </table>
