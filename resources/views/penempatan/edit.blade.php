            ...
            <div class="col-md-12 grid-margin">
              <div class="row">
                <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                  <h3 class="font-weight-bold">Edit Penempatan</h3>
                </div>
              </div>

            <div class="card shadow mb-4">
            <div class="card-body">
            <form name="saveform" action="{{route('updatepenempatan', $data->id)}}" method="post" onsubmit="return validateForm()">
              @csrf
            <div class="form-group mb-4">
            <label for="" class="form-label" style="color:black;">Kode Orange</label>
            <input name="kode_orange" type="number" min="0"  class="form-control " style="border-color: #01004C;" value="{{$data -> kode_orange}}" oninput="validasiNumber(this)"/>
            </div>

            <div class="form-group mb-4">
                    <label for="" class="form-label" style="color:black;">Nama Unit Kerja</label>
                    <input name="nama_unit_kerja" type="text"  class="form-control " style="border-color: #01004C;" value="{{$data -> nama_unit_kerja}}" />
                    </div>
                              

            <div class="form-group mb-4">
                <label for="" class="form-label" style="color:black;">Wilayah</label>
                <input name="wilayah" type="text"  class="form-control " style="border-color: #01004C;" value="{{$data -> wilayah}}" />
                </div>


                <div class="form-group mb-4">
                    <label for="" class="form-label" style="color:black;">Divisi</label>
                    <input name="divisi" type="text"  class="form-control " style="border-color: #01004C;" value="{{$data -> divisi}}" />
                    </div>

                    <div class="form-group mb-4">
                    <label for="" class="form-label" style="color:black;">KCU Induk</label>
                    <input name="kcu_induk" type="text"  class="form-control " style="border-color: #01004C;" value="{{$data -> kcu_induk}}" />
                    </div>

                
                    <div class="form-group mb-4">
                    <label for="" class="form-label" style="color:black;">Kode Cabang Pembayaran</label>
                    <input name="kode_cabang_pembayaran" type="number"  min="0" class="form-control " style="border-color: #01004C;" value="{{$data -> kode_cabang_pembayaran}}" oninput="validasiNumber(this)"/>
                    </div>
                    ...
                    <input name="rcc_pembayaran" type="number" min="0" class="form-control " style="border-color: #01004C;" value="{{$data -> rcc_pembayaran}}" oninput="validasiNumber(this)"/>
                    </div>
...
function validasiNumber(input) {
    ...
    input.value = input.value.replace(/\D/g, '');
                    ...
                    <input name="singkatan_divisi" type="text"  class="form-control " style="border-color: #01004C;" value="{{$data ->singkatan_divisi}}" />
                    </div>

                    <div class="form-group mb-4">
                    <label for="" class="form-label" style="color:black;">Kode SLID</label>
                    <input name="kode_slid" type="text"  class="form-control " style="border-color: #01004C;" value="{{$data ->kode_slid}}" />
                    </div>
                    
                    <div class="form-group mb-4 ml-4">
    <div class="form-check">
        <input name="flexCheckIndeterminate" class="form-check-input" type="checkbox" value="Yes" id="flexCheckIndeterminate" 
               onclick="this.value=this.checked ? 'Yes' : 'No';" 
               {{ $data->hitung_tunjangan == 'Yes' ? 'checked' : '' }}>
