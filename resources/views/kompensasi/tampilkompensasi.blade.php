            ...
            <div class="col-md-12 grid-margin">
                <div class="row">
                    <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                        <h3 class ="font-weight-bold">Detail {{$kompensasi -> judul}}</h3>
                    </div>
                </div>
                ...
                <div class="row mb-4">
    <div class="col-md-12">
    <a href="{{ route('download.kompensasi', ['bulan' => $kompensasi->bulan, 'tahun' => $kompensasi->tahun, 'status_kompensasi' => $kompensasi->status_kompensasi,  'dataKompensasi' => json_encode($dataKompensasi)] ) }}" class="btn btn-success">Download Kompensasi</a>
    </div>
</div>

<form id="closekompensasiForm" action="{{ route('close.kompensasi', $kompensasi->id)}}" method="POST" style="display: none;">
    @csrf
    <input type="hidden" name="dataKompensasi" id="dataKompensasiInput">
</form>

<script>
  function closekompensasi() {
    var dataKompensasi = @json($dataKompensasi); // Assuming $dataKompensasi is passed from your backend
    document.getElementById('dataKompensasiInput').value = JSON.stringify(dataKompensasi);
    document.getElementById('closekompensasiForm').submit();
}

</script>
<div class="row mb-4">
    <div class="col-md-12">
        @if ($kompensasi->status_kompensasi == 'Closing')
        <button type="button" class="btn btn-primary" disabled>Closing Kompensasi</button>
        @else
        <button type="button" class="btn btn-primary" onclick="closekompensasi()">Closing Kompensasi</button>
        @endif
    </div>
</div>
                ...
                <div class="card shadow mb-4">
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            ...
                            $no = 1;
                            @endphp
                                @foreach ($dataKompensasi as $kompensasi )
                                <tr>
                               <td>{{$no++}}</td>
                              <td>{{$kompensasi['nik']}}</td>
                              <td>{{$kompensasi['nama']}}</td>
                              <td>{{$kompensasi['namapenempatan']}}</td>
                              <td>{{$kompensasi['gaji']}}</td>
                              <td>{{$kompensasi['tunjangan']}}</td>
                              <td>{{$kompensasi['tanggalawal']}}</td>
                              <td>{{$kompensasi['tanggalakhir']}}</td>
                              <td>{{$kompensasi['masakerja']}}</td>
                              <td>{{ 'Rp ' . number_format($kompensasi['totalkompensasi'], 0, ',', '.') }}</td>
                               </tr>
                               @endforeach
                                </tbody>
                            </table>
