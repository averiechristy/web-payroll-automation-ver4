    ...
    <div class="content-wrapper">
        <div class="row">
            <div class="col-md-12 grid-margin">
                ...
                <div class="row mb-4">
    <div class="col-md-12">
        <a href="{{ route('download.kompensasi', ['bulan' => $kompensasi->bulan, 'tahun' => $kompensasi->tahun]) }}" class="btn btn-success">Download Kompensasi</a>
    </div>
</div>
                ...
                <div class="card shadow mb-4">
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            ...
                            $no = 1;
                            @endphp
                                @foreach ($detailkompensasi as $detail )
                                <tr>
                               <td>{{$no++}}</td>
                                <td>{{$detail->karyawan->nik}}</td>
                                <td>{{$detail->karyawan->nama_karyawan}}</td>
                                <td>{{$detail->karyawan->penempatan->nama_unit_kerja}}</td>
                                <td>{{ 'Rp ' . number_format($detail->gaji, 0, ',', '.') }}</td>
                                <td>{{ 'Rp ' . number_format($detail->tunjangan, 0, ',', '.') }}</td>
                                <td>{{$detail->karyawan->tanggal_awal_kontrak}}</td>
                                <td>{{$detail->karyawan->tanggal_akhir_kontrak}}</td>
                                <td>{{$detail->masa_kerja}}</td>
                                <td>{{ 'Rp ' . number_format($detail->total_kompensasi, 0, ',', '.') }}</td>
                               </tr>
                               @endforeach
                                </tbody>
                            </table>
