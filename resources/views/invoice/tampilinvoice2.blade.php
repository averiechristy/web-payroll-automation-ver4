...
<form id="closeinvoiceForm" action="{{ route('close.invoicekedua', $invoice->id)}}" method="POST" style="display: none;">
    @csrf
    <input type="hidden" name="dataInvoicedua" id="datainvoice">
</form>

<script>
  function closepayroll() {
    var dataInvoicedua = @json($dataInvoicedua); // Assuming $datainvoice is passed from your backend
    document.getElementById('datainvoice').value = JSON.stringify(dataInvoicedua);
                ...
                <div class="card shadow mb-4">
                    <div class="card-body">
                        ...
                        <th>Keterangan</th>
                                ...
                                @endphp
                                <tbody>
                                    @foreach ($dataInvoicedua as $index=>$data )
                                    <tr>
                                        <td>{{$no ++}}</td>
                                        <td>{{$data['nama']}}</td>
