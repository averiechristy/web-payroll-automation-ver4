            ...
            <div class="col-md-12 grid-margin">
                <div class="row">
                    <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                        <h3 class ="font-weight-bold">Detail {{$lembur -> judul}}</h3>
                    </div>
                </div>
                ...
                <div class="row mb-4">
    <div class="col-md-12">
        <a href="{{ route('download.lembur', ['organisasi_id' => $lembur->organisasi_id,'bulan' => $lembur->bulan, 'tahun' => $lembur->tahun]) }}" class="btn btn-success">Download Laporan Lembur</a>
    </div>
</div>
</div>
  ...
  <td >Biaya Jam 2</td>
    ...
    <td >Biaya Jam 4</td>
                                ...
                                <tbody>
                                    ...
                                    $No = 1;
                                    @endphp
                                    @foreach ($detaillembur as $detail )                                    
                               <tr>
                       <td>{{$No++}}</td>
                       <td>{{$detail->karyawan->nik}}</td>
                       <td>{{$detail->karyawan->payroll_code}}</td>
                       <td>{{$detail->karyawan->nama_karyawan}}</td>
                       <td>{{$detail->karyawan->jabatan}}</td>
                       <td>{{$detail->karyawan->penempatan->organisasi->organisasi}}</td>
                       <td>{{$detail->karyawan->leader}}</td>
                       <td>{{$detail->karyawan->status_karyawan}}</td>
                       <td>{{$detail->total_jam_pertama_hari_kerja}}</td>
                       <td>{{ 'Rp ' . number_format($detail->total_biaya_jam_pertama_hari_kerja, 0, ',', '.') }}</td>
                       <td>{{$detail->total_jam_kedua_hari_kerja}}</td>
                       <td>{{ 'Rp ' . number_format($detail->total_biaya_jam_kedua_hari_kerja, 0, ',', '.') }}</td>
                       <td>{{$detail->total_jam_kedua_hari_libur}}</td>
                       <td>{{ 'Rp ' . number_format($detail->total_biaya_jam_kedua_hari_libur, 0, ',', '.') }}</td>
                       <td>{{$detail->total_jam_ketiga_hari_libur}}</td>
                       <td>{{ 'Rp ' . number_format($detail->total_biaya_jam_ketiga_hari_libur, 0, ',', '.') }}</td>
                       <td>{{$detail->total_jam_keempat_hari_libur}}</td>
                       <td>{{ 'Rp ' . number_format($detail->total_biaya_jam_keempat_hari_libur, 0, ',', '.') }}</td>
                       <td>{{$detail->total_jam}}</td>
                       <td>{{$detail->total_biaya_lembur}}</td>
                       </tr>    
                       @endforeach
                                </tbody>
                            </table>
