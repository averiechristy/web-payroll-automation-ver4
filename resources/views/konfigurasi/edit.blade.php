            ...
            <div class="col-md-12 grid-margin">
                ...
                <div class="row">
                    <div class="col-12 col-xl-8 mb-4 mb-xl-0">
                        <h3 class="font-weight-bold">Edit Konfigurasi</h3>
                    </div>            
                </div>

                <div class="card shadow mb-4">
                    <div class="card-body">
                        <form name="editform" action="{{route('updatekonfigurasi', $data->id)}}" method="post" onsubmit="return validateForm()">
                            @csrf
                            ...
                            <div class="form-group mb-4">
                                ...
                                <select name="organisasi_id" id="organisasi" class="form-control form-select-lg mb-3" aria-label=".form-select-lg example" style="color:black;">
                                    ...
                                    @foreach ($organisasi as $item)
                                        <option value="{{$item->id}}" {{$item->id == $data->organisasi_id ? 'selected' : ''}}>{{$item->organisasi}}</option>
                                    @endforeach
                                </select>
                            </div>
                            ...
                            <div class="form-group mb-4 ml-4">
                                <div class="form-check">
                                    <input class="form-check-input" type="checkbox" id="pilihSeluruhPenempatan" name="pilih_seluruh_penempatan" checked>
                                ...
                                <div class="card-body">
                                    <div class="form-group">
                                        ...
                                        <div class="form-check ml-4">
                                            <input class="form-check-input" type="checkbox" id="hitungTunjangan" name="hitungtunjangan" {{$data->hitung_tunjangan == 'Yes' ? 'checked' : ''}}>
                                    ...
                                    <div class="form-group mt-4">
                                        ...
                                        <div class="form-check ml-4">
                                            <input class="form-check-input" type="radio" name="perhitunganpayroll" id="harikerja" value="harikerja" {{$data->perhitungan_payroll == 'harikerja' ? 'checked' : ''}}>
                                            <label class="form-check-label" for="harikerja">
                                                Hari Kerja
                                            </label>
                                        </div>
                                        <div class="form-check ml-4">
                                            <input class="form-check-input" type="radio" name="perhitunganpayroll" id="calendar" value="kalender" {{$data->perhitungan_payroll == 'kalender' ? 'checked' : ''}}>
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="form-check ml-4">
                                            <input class="form-check-input" type="checkbox" id="buatInvoice" name="buatinvoice" {{$data->buat_invoice == 'Yes' ? 'checked' : ''}}>
...
<script>
    $(document).ready(function() {
        ...
        // Load penempatan when the page loads
        loadPenempatan({{ $data->organisasi_id }});

        $('#organisasi').on('change', function() {
            var organisasiID = $(this).val();
            loadPenempatan(organisasiID);
        });
        ...
        var detailPenempatanIDs = @json($detail->pluck('penempatan_id')->toArray());

        function loadPenempatan(organisasiID) {
            if(organisasiID) {
                $.ajax({
                    ...
                    success:function(data) {
                        ...
                        $.each(data, function(key, value) {
                            var isChecked = detailPenempatanIDs.includes(value.id) ? '' : 'checked';
                            $('#penempatanCheckboxes').append('<div class="form-check"><input class="form-check-input penempatan-checkbox" type="checkbox" id="penempatan_'+ value.id +'" name="penempatan_ids[]" value="'+ value.id +'" '+isChecked+'><label class="form-check-label" for="penempatan_'+ value.id +'">'+ value.nama_unit_kerja +'</label></div>');
                        });
                    }
                });
            } else {
    ...
    function validateForm() {
        let organisasi = document.forms["editform"]["organisasi_id"].value;
        let hitungTunjangan = document.forms["editform"]["hitungtunjangan"].checked;
        let perhitunganPayrollHariKerja = document.forms["editform"]["perhitunganpayroll"][0].checked;
        let perhitunganPayrollKalender = document.forms["editform"]["perhitunganpayroll"][1].checked;
